            ...
            with col:
                ...
                st.markdown(
                    ...
                    display: flex; flex-direction: column; align-items: center;">
                        <strong>{word}</strong>
                        <div style="margin-top: 10px; font-size: 18px;">{st.session_state.correct_answers.get(i, '')}</div>
                        <div style="margin-top: 20px; display: flex; justify-content: space-between; width: 100%;">
                            <div style="flex: 1; display: flex; justify-content: center;">
                                <button style="width: 60px; height: 30px;" onclick="window.location.reload()">对</button>
                            </div>
                            <div style="flex: 1; display: flex; justify-content: center;">
                                <button style="width: 60px; height: 30px;" onclick="window.location.reload()">错</button>
                            </div>
                        </div>
                    </div>
                ...
                # 将“对”和“错”按钮放在同一行，确保按钮紧靠且居中
                col1, col2 = col.columns([1, 1])  # 创建两个子列，比例相同
                with col1:
